...
var (
	...
	Device_Clinets = make(map[string]*Device)
...
func ConnectModbus(ip string, port string) (*modbus.ModbusClient, error){
	
	// 단순히 client라는 struct을 만들어 주는 것이라서 여기서는 err가 안남
...
func Start(ip string, length int, port int) error {
	if length != 0 {
		for portNum := port; portNum < port + length; portNum++ {
			...
			Client, err := ConnectModbus(ip, strconv.Itoa(portNum))
	...
	return fmt.Errorf("최소 1개이상의 디바이스를 연결해야 합니다.")
}
...
func main() {
	...
	app.Post("/", func(c fiber.Ctx) error {
		...
		err = Start(result.Ip, result.Dvc_len, result.Port)
